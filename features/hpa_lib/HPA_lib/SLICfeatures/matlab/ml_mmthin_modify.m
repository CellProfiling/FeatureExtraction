function [ima_skel] = ml_mmthin_modify(objbin)

% mmthin_modify (objbin) 
%
...
min_col = min(J);
max_col = max(J);
bin_image = objbin(min_row : max_row, min_col : max_col);
degrees = 45;
...
[r c] = size(objbin);
ima_skel = zeros(r, c);
ima_skel(min_row:max_row, min_col:max_col) = double(bin_image);
