function [varargout] = rdir_list(rootdir,varargin)
%
...
elseif strcmp(wildpath,'**'), % a double wild directory means recurs down into sub directories
    ...
    D = rdir_list([prepath postpath(2:end)]);
    ...
    for ii = 1:length(tmp),
        if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
            Dt = [Dt; rdir_list([prepath tmp(ii).name filesep wildpath postpath])];
        end;
    end;
    D = [D; Dt];
    ...
    for ii = 1:length(tmp),
        if (tmp(ii).isdir && ~strcmpi(tmp(ii).name,'.') && ~strcmpi(tmp(ii).name,'..') ),
            D = [D; rdir_list([prepath tmp(ii).name postpath])];
        end;
    end;
end;
