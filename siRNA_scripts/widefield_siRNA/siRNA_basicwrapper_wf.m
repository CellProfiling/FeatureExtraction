function [featmat_old, fnames_old] = siRNA_basicwrapper_wf(inpath,featnamepath,featfilename)
...
%
%Updated by: 
%D.P. Sullivan 2018,05,23 - Adapted for new workflow and widefield
%microscope (Lieca DMI8) 
...
last_splitfeat_file = [outputdir,filesep,fnames{end}(1:end-1),'_splitfeats.mat'];

%%%MAKE FEATURE MATRIX%%%
% if ~exist(masteroutname,'file')
if ~exist(masteroutname,'file') || ~exist(last_splitfeat_file, 'file')
...
int_whole = load([outputdir,filesep,fnames{end-2}(1:end-1),'_splitfeats.mat']);
int_nuc = load([outputdir,filesep,fnames{end-1}(1:end-1),'_splitfeats.mat']);
%int_cyto = load([outputdir,filesep,fnames{8}(1:end-1),'_splitfeats.mat']);
int_cyto = load(last_splitfeat_file);
...
csvwrite([outputdir, filesep, 'out_', featfilename], featmat_old);
